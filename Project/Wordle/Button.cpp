...
void RectangleButton::newButton(int x,int y,int width,int height,const char* String){
...
void RectangleButton::DrawButton(){
	...
	for (auto buttons:button){
		int r,g,b,b2;
		r=rand()%255,g=rand()%255,b=rand()%255,b2=rand()%255;
		if (buttons.state.pressed) {
			color = buttons.pressedColor;
		}
		else color = buttons.normalColor;
		if (buttons.state.hovered) color = interpColor(color, BLACK, 0.125f);
		color_t color2 = EGEARGB(r,g,b,b2);
		setfillcolor(color);
		ege_fillrect(buttons.x,buttons.y,buttons.width,buttons.height);
		if (buttons.state.focused) {
			int edge=3;
			setlinewidth(edge);
			setcolor(interpColor(color, BLACK, 0.25f));
			ege_rectangle(buttons.x-edge,buttons.y-edge,buttons.width+2*edge,buttons.height+2* edge);
		}
		setcolor(WHITE);
		xyprintf((2*buttons.x+buttons.width)/2, (2*buttons.y+buttons.height)/2, "%s", buttons.text);
	}
}

int RectangleButton::SearchButton(int x,int y){
	int n=button.size();
	for (int i=0;i<n;i++){
		if (insideButton(button[i],x,y)) return i;
	}
...
void RectangleButton::PutButton(int& Select){
	bool mouseMoved = false;
	while (mousemsg()){
		mouse_msg msg=getmouse();
		if (msg.is_left()){
...
color_t RectangleButton::interpColor(color_t colorA, color_t colorB, float t){
	if (t > 1) t = 1;
	else if (t < 0) t = 0;
	int r = roundf((1 - t) * EGEGET_R(colorA) + t * EGEGET_R(colorB));
	int g = roundf((1 - t) * EGEGET_G(colorA) + t * EGEGET_G(colorB));
