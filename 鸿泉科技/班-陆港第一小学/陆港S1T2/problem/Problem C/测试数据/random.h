...
void iprint(T &x, char y = ' ') {
	freopen(InputFileName.c_str(), "a", stdout);
	cout << x << y;
	fclose(stdout);
}
...
void oprint(T &x, char y = ' ') {
	freopen(OutputFileName.c_str(), "a", stdout);
	cout << x << y;
	fclose(stdout);
}

void initcase() {
	InputFileName = FileName + to_string(caseNumber) + "." + InputFileSuffix;
	OutputFileName = FileName + to_string(caseNumber) + "." + OutputFileSuffix;
}
