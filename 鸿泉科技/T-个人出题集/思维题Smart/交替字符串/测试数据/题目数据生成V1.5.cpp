...
//|---------------------------------------------|
int ll = 8, rr = 8; //Test Cases Number
//|---------------------------------------------|
void solve(int number) {
	...
	//|---------------------------------------------|
	string s;
	int n = mrand(990, 1e3, 0, -1);
	int aaaaa = rand() % 52;
	int bbbbb = rand() % 52;
	while (bbbbb == aaaaa) {
		bbbbb = rand() % 52;
	}
	char xxxxx = aaaaa < 26 ? 'a' + aaaaa : 'A' + aaaaa - 26;
	char yyyyy = bbbbb < 26 ? 'a' + bbbbb : 'B' + bbbbb - 26;
	int last = 0;
	for (int i = 0; i < n; i++) {
		int pppp;
		int epss = rand() % 10000;
		if (epss > 9995) pppp = last;
		else pppp = last ^ 1;
		if (pppp) s.push_back(xxxxx);
		else s.push_back(yyyyy);
		last = pppp;
	}
	print(s);
	long long ans = 0;
