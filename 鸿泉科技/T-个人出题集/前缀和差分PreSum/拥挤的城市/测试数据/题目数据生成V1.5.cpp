...
void solve(int number) {
	...
	int n, m, l, r;
	n = mrand(80000, 100000);
	m = mrand(80000, 100000, 0, 2);
	vector<long long> a(n), pre(n + 1), suf(n + 1);
	a[0] = mrand(1, 1000);
	a[1] = mrand(1001, 1500);
	for (int i = 2; i < n; i++) {
		int xxx = mrand(10, 1000, 0, -1);
		while (xxx == a[i - 1] - a[i - 2]) {
			xxx = mrand(1, 10, 0, -1);
		}
		a[i] = a[i - 1] + xxx;
		print(a[i]);
	...
	while (m--) {
		...
		r = mrand(2, n, 0, 2);
		if (l < r) {
			ans.push_back(pre[r - 1] - pre[l - 1]);
		} else {
			ans.push_back(suf[r] - suf[l]);
		}
	}
    //|---------------------------------------------|
